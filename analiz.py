...
# 4. Günün dönemini belirleyen fonksiyon
def donem_belirle(saat):
    if 5 <= saat < 12:
        return 'Sabah'
    elif 12 <= saat < 17:
        return 'Öğle'
...
df['Gunun_Donemi'] = df['Hour'].apply(donem_belirle)
print(df[['Hour', 'Gunun_Donemi']].head(10))
...
print(f"Sipariş Başına Ortalama Tutar: {(toplam_ciro / len(df)):.2f} ")
